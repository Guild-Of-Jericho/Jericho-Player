...
export default class ytMusicEx {
	...
	static async spSearch(title: string, duration?: string, artist: string = '') {
		...
		return await searchMusics((artist ? +`${artist.trim()} - ` : '') + title.trim()).then(
			(tracks) => ytMusicEx.#compareTracks(tracks, title, artist, duration),
		);
	}

	static async pYTList(json?: YouTubePlayList, ext?: extractor, meta?: any): Promise<Playlist> {
			...
			playlist = new Playlist(
				{
					...
					author: {
						...
						image: json.channel?.icons?.sort((a, b) => a?.width - b?.width)?.pop()?.url,
					},
