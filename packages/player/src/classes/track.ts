...
export default class Track {
	...
	async download(extractor: string = this.extractor as string) {
		switch (extractor?.toLowerCase()?.trim()) {
			case 'youtube':
				return await stream((this.streamUrl ?? this.url) as string, { quality: 3 })?.catch(
					() => undefined,
				);
			case 'spotify':
				...
				return await searchMusics(
					...
					.then(
						async (track) =>
							await stream('https://music.youtube.com/watch?v=' + track?.youtubeId, {
								quality: 3,
							})?.catch(() => undefined),
					);
			case 'soundcloud':
					...
					return new Promise((resolve) => {
						try {
							if (this.url?.startsWith('https://'))
								https.get(this.url, (res) => resolve(res));
							else if (this.url?.startsWith('http://'))
								http.get(this.url, (res) => resolve(res));
						} catch {
							resolve(undefined);
						}
					});
	...
	get streamUrl(): string {
		if (['youtube', 'soundcloud'].includes(this.extractor!?.toLowerCase()?.trim()))
