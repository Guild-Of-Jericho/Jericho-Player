...
export default class soundCloudEx {
	...
	static async nonApiGet(query: string, ext?: extractor, meta?: any): Promise<Track | Playlist> {
		if (!isUrl(query)) return await soundCloudEx.nonApiSearch(query, ext, meta);
		return await soundCloudEx.nonApiTrack(query, ext, meta);
	}
	...
	static async nonApiSearch(query: string, ext?: extractor, meta?: any) {
		...
		let track = await scdClientGen.client
			...
			.then((sR) => sR?.collection?.filter((tR) => tR?.kind?.trim() === 'track')?.shift())
			.then(
				(json: any) =>
					new Track(
						{
