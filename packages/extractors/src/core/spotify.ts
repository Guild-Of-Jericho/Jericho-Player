...
export default class spotifyEx {
	...
	static async getNonApi(query: string, ext?: extractor, meta?: any): Promise<Track | Playlist> {
		try {
			if (!isUrl(query)) return await spotifyEx.apiSearchTrack(query, ext, meta);
	...
	static getNonApiTrack(json: Record<string, any>, ext?: extractor, meta?: any): Track {
	...
	static getNonApiPlaylist(json: Record<string, any>, ext?: extractor, meta?: any): Playlist {
		if (!json?.id) return undefined;
	...
	static async apiSearchTrack(query: string, ext?: extractor, meta: any = {}): Promise<Track> {
