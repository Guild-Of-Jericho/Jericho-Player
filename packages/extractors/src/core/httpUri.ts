...
export default class httpUriEx {
	...
	static async getHttp(url: string, ext?: extractor, meta?: any): Promise<Track> {
		if (!isUrl(url)) return undefined;
		else if (httpUriEx.regex.audio.test(url))
			return await httpUriEx
				.getMetadata(url)
				.then((data) => new Track(data as httpMetadataType, meta))
				.then((track) => {
			...
			fluentFfmpeg.ffprobe(url.trim(), (err, data) => {
				if (err) resolve(undefined);
					...
					resolve({
						...
						duration: {
							hR: ms2hu(data?.format?.duration! * 1000) as string,
